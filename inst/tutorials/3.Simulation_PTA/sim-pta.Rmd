...
temp1 <- PM_data$new()$
  addEvent(id = 1, time = 0, dose = 500, addl = 5, ii = 24, wt = 75, africa = 0, age = 25, gender = 1, height = 170)$
  addEvent(id = 1, time = 144, out = -1)$
  addEvent(id = 2, time = 0, dose = 1000, addl = 5, ii = 24, wt = 75, africa = 0, age = 25, gender = 1, height = 170)$
  addEvent(id = 2, time = 144, out = -1)$
  addEvent(id = 3, time = 0, dose = 250, addl = 11, ii = 12, wt = 75, africa = 0, age = 25, gender = 1, height = 170)$
  addEvent(id = 3, time = 144, out = -1)$
  addEvent(id = 4, time = 0, dose = 500, addl = 11, ii = 12, wt = 75, africa = 0, age = 25, gender = 1, height = 170)$
...
Welcome to the Pmetrics simulation tutorial. This tutorial will briefly introduce you to the R programming language and the Pmetrics simulation framework.
The main topics of this tutorial are:

- Understanding data templates
- Simulating from prior model fitting runs
- Simulating from literature extractions
- Conducting probability of target attainment
...
- If you have not done so already, you can obtain everything you need by following these [instructions](http://www.lapk.org/Pmetrics_install.php){target="\_blank"}. **Important**: For this workshop we are using a beta version of Pmetrics, v. 2.1.0. Prior versions will not work. Replace the command in step 4 with `remotes::install_github("LAPKB/Pmetrics@dev")`.

## Foundations

This tutorial was designed to accompany a workshop, but may be completed independently. We assume that you have completed the [pre-workshop exercises](http://www.lapk.org/workshop/Mar2024/Pre-Workshop.html){target="\_blank"}.
...
- You should have loaded the Pmetrics library in R with `library(Pmetrics)` and created a _Workshop_ folder within your _Pmetrics_ folder using `PMtree("Workshop")`.
- Your _Workshop_ folder will contain subfolders as described in the help for [PMtree](https://lapkb.github.io/Pmetrics/reference/PMtree.html){target="\_blank"}.

**Let's get started!**

## Simulation from prior model fitting

Recall that Pmetrics always requires a data file and a model file. Once it has these, it fits the data to the model to estimate the probability distributions of the parameter values in the model equations. When you load these results with [PM_load](https://lapkb.github.io/Pmetrics/reference/PM_load.html){target="\_blank"}, you can use them for simulations among other things.

### Look at PM_result objects

Pmetrics comes with the results of a run built in for you to use. Look at the description.

```{r npex, exercise = TRUE}
?NPex
```

_NPex_ is a [PM_result](https://lapkb.github.io/Pmetrics/reference/PM_result.html){target="\_blank"} object. Click on the link to see a description of the fields and methods contained within _NPex_.

```{r npex-quiz}
quiz(
  caption = "Understanding PM_result objects",
  question("R6 objects like NPex, a PM_result object, can have data fields and methods.",
           answer("TRUE", correct = TRUE),
           answer("FALSE"),
           allow_retry = TRUE
  ),
  question("How would you access the simulation method for NPex?",
           type = "learnr_text",
           answer_fn(\(x){
             x2 <- tidyup(x)
             if(x2 == "NPex$sim()"){return(correct())}
             if(x2 == "NPex$sim"){return(incorrect("Don't forget trailing parentheses."))}
             if(x2 == "NPex"){return(incorrect("You haven't specified a method."))}
             return(incorrect("Hint: NPex$..."))
           }),
           allow_retry = TRUE
  )
)
...
This is the easiest way to simulate, because you've already made your data and model objects. Note that _PM_result_ objects also contain data and model fields, which in turn contain your original data and model objects.

### Simulate from PM_result

Now that you know how to access the simulation method attached to a _PM_result_ object that is
generated by [PM_load](https://lapkb.github.io/Pmetrics/reference/PM_load.html){target="\_blank"}, let's use it!

Like all "engines" in Pmetrics, the simulator needs a model, and some data. In this case the data serve as a template for the doses, covariates, outputs and timing of samples that you want to simulate. The simulator also needs to know the distributions you want to use for the parameter values in the model equations. If you want to use the same model, data, and parameter value distributions as you used in the original fitting, it's very easy to access those in the _PM_result_ object.

- The model is in the `$model` field.
- The data are in the `$data` field.
- The parameter values are in the `$final` field, and specifically the `$final$popMeans` for the mean values, and `$final$popCov` for the covariances.

Here's how simple the code can be:

`NPex$sim()`

Arguments that can go within the parentheses are all supplied to the [SIMrun](https://lapkb.github.io/Pmetrics/reference/SIMrun.html){target="\_blank"} function, which is the function that the `$sim()` method calls. `SIMrun` requires the following arguments at minimum:

- `poppar` are parameter value distributions
- `model` is the model object
- `data` is the data template

However, since they are all in the _PM_result_ object as discussed, you don't have to supply them!

```{r simrun-question}

  question("Look at the help for SIMrun with the link above. Which of the following are arguments to SIMrun? Choose all that apply.",
           answer("limits", correct = TRUE),
           answer("covariate", correct = TRUE),
           answer("pred.type", correct = FALSE),
           answer("nsim", correct = TRUE),
           answer("outeq", correct = TRUE),
           answer("indpts", correct = FALSE),
           allow_retry = TRUE
  )
```

### Your first simulation - create the data template

We're going to simulate in R for this. The first task is to create the data template.

There are two ways to create the data template. The first is to use a spreadsheet program like Excel. The second is to create a template on the fly in R. Here we'll look at the first option.

Be sure to look at the article on Pmetrics [data objects](https://lapkb.github.io/Pmetrics/articles/data.html){target="\_blank"} for details on the column headers and meanings of the columns.

### Create data template in a spreadsheet program

The task is to create a template data file with four different oral dose regimens:
...
Simulate the time of the last observation at 144 hours for each.

1. Open the program, e.g. Excel.
2. Name your columns: ID, TIME, DOSE, ADDL, II, OUT, WT, AFRICA, AGE, GENDER, HEIGHT. We won't be using the covariates, but since they are in the model within `NPex` we need to include them in our template.
3. Make 4 subjects, each with the following.

- A DOSE at TIME 0 according to our goal doses above.
- The right number of ADDL doses so that the last dose is given at TIME = 144 with dosing at an appropriate II interval.
- Follow the dosing by one OUT with value -1 at time 144 (24 hours after the last dose)
- Fill in WT = 75, AFRICA = 0, AGE = 25, GENDER = 1, HEIGHT = 170 for every row.

4. Save the file as "temp.csv". If you made a `/src` folder in the pre-workshop exercises, put the file there. If your spreadsheet uses semicolons (;) as field separators, you'll need to update Pmetrics with `setPMoptions(sep = ";")`.
5. Read the file into R with `dat1 <- PM_data$new("temp.csv")`. Be sure that temp.csv is in your current working directory in R.

### Run the simulation in R

Use all default simulation arguments except add the following:

- `limits = c(0,3)` Simulated parameter values within the range of 0 \* _lower_ and
  3 \* _upper_ will be accepted. It puts some physiologic boundaries on simulated values.
- `predInt = 1` Output will be simulated every hour from time 0 to the last time in each subject's record (i.e. 144).

Assign the output to an object called `sim1`. Try writing the simulation statement in R yourself, and after check here to make sure it's correct.

<details>
<summary>Check your code</summary>
`sim1 <- NPex$sim(data = dat1, predInt = 1, limits = c(0,3))`
</details>

Explore your new simulation in R.

<details>
<summary>Plot the 3rd simulation with default quantiles.</summary>
`sim1$plot(at = 3)`
</details>

<details>
<summary>Plot only the 25th and 75th percentiles of the simulated profiles for the 2nd simulation.</summary>
`sim1$plot(at = 2, line = list(probs = c(0.25, 0.75)))`
</details>
...
quiz(
  ...
  question("What type of distribution do the interindividual random parameter (eta, $\\eta$) values have?",
           ...
           allow_retry = TRUE),
  question("Which primary parameters have interindividual variance (IIV)?",
           answer("$\\theta_1$ (CL_int)"),
           answer("$\\theta_2$ (CL_slope)"),
           answer("$\\theta_3$ (V_slope)", correct = TRUE),
           allow_retry = TRUE,
           incorrect = "IIV is on CL, not CL_int and CL_slope",
           correct = "Yes, IIV is on V_slope, which defines V, and on CL, but not on CL_int and CL_slope."
  )
)
```
...
quiz(caption = "Extracting necessary values",
     ...
     question("What is the typical value of $\\theta_2$ (CL_slope)?",
              ...
              answer_fn(\(x){
                x2 <- tidyup(x)
                if(!is.na(stringr::str_extract(x2, stringr::regex("9.6", ignore_case = TRUE)))){
                  return(correct())
                }
                return(incorrect())
              }),
              incorrect = "It should be 9.6."
     ),
     question("What is the value of $\\omega$ for the standard deviation of the $\\eta$-distribution around CL? Hint: use the approximation formula above and the reported CV%. Round to three decimal places.",
              type = "learnr_text",
              answer_fn(\(x){
                x2 <- tidyup(x)
                if(!is.na(stringr::str_extract(x2, stringr::regex(".277", ignore_case = TRUE)))){
                  return(correct())
                }
                return(incorrect())
              }),
...
As you recall, there are two ways to create the data template. We already used the first to create a spreadsheet in a program like Excel. The second is to create a template on the fly in R. Let's look at the spreadsheet method again, then compare it to a template on the fly.

Again, if you need a data format refresher, look at the article on Pmetrics [data objects](https://lapkb.github.io/Pmetrics/articles/data.html) for details on the column headers and meanings of the columns.
...
1. Open the _Workshop.R_ file in the _Rscript_ folder of the _Workshop_ project folder you made with `PMtree` previously. This file is designed to get you started building a model from data. In this case, we are developing simulations from the literature.
2. Copy and paste your full data template creation code into your Rscript and save it.
3. You can also copy and paste the file you made in Option 1 into the _Sim_ folder. Call that file "simtpt.csv", short for "simulation template".
4. In your _Workshop.R_ script, save the object you created as a second template file.

- Change the working directory with [setwd](https://rdrr.io/r/base/getwd.html){target="\_blank"}.

* You can first execute line 6 to change the working directory to the _Runs_ folder in your project.
* Then use this command to switch to the _Sim_ folder: `setwd("../Sim")`

- Use `dat$write("simtpt2.csv")` to save the `PM_data` object you made into the _Sim_ folder of your project.
...
1. Ensure that Pmetrics is loaded in R with `library(Pmetrics)`. This is the first line of code in your _Workshop.R_ file in your project tree.
2. Use [build_model](https://lapkb.github.io/Pmetrics/reference/build_model.html){target="\_blank"} to launch the app.
...
answer_fn(\(x){
  x2 <- tidyup(x)
  if(!is.na(stringr::str_extract(x2, stringr::regex("CL=\\(5\\.05\\+9\\.6\\*ccr/89\\)\\*exp\\(etaCL\\)", ignore_case = TRUE)))){
    return(correct())
  }
  return(incorrect())
}),
...
A file named "model.txt" is saved in your current working directory. You can double check this in R with [getwd](https://rdrr.io/r/base/getwd.html){target="\_blank"}.
...
At last we are ready to simulate. As described in [Simulation](https://lapkb.github.io/Pmetrics/articles/simulation.html){target="\_blank"} vignette, there are two ways to run the simulator.
...
```{r echo = TRUE, eval = FALSE}
sim1 <- PM_sim$new(poppar, model, data,...)
```
...
The [Simulation](https://lapkb.github.io/Pmetrics/articles/simulation.html){target="\_blank"} vignette and the help for the underlying simulator function [SIMrun](https://lapkb.github.io/Pmetrics/reference/SIMrun.html){target="\_blank"} detail all the arguments you can use to change the behavior of the simulator. Here, we'll focus on two:
...
You'll see that it is a [PM_simlist](https://lapkb.github.io/Pmetrics/reference/PM_simlist.html){target="\_blank"}, which contains a `data` field and several methods. The `data` field contains a list of all the [PM_sim](https://lapkb.github.io/Pmetrics/reference/PM_sim.html){target="\_blank"} objects, one for each of the three template dosing regimens in your data template file.

To plot from a [PM_simlist](https://lapkb.github.io/Pmetrics/reference/PM_simlist.html){target="\_blank"} object, call the method and use the `at` argument to choose which of the list of `PM_sim` objects you want to plot.
...
The help for [plot.PM_sim](https://lapkb.github.io/Pmetrics/reference/plot.PM_sim.html){target="\_blank"} documents all the options for plotting. Pmetrics uses the `plotly` package for default plotting. Plotly is a package originally developed in Python but ported to R. Help is generally available by searching the internet for "R plotly ..." as well as help files for any plotly function. Try `?plot_ly` and then click `Index` at the bottom of the page. You can also use the [schema](https://rdrr.io/cran/plotly/man/schema.html){target="\_blank"} command to invoke an expandable list of all plotly plots and arguments. Lastly, view the Pmetrics article on [plotly](https://lapkb.github.io/Pmetrics/articles/data.html){target="\_blank"}

## A little more advanced plotting

There is no pre-defined Pmetrics plot function to examine the simulated parameter values, but of course in R you can do this. Remember that sim1 is a list of [PM_sim](https://lapkb.github.io/Pmetrics/reference/PM_sim.html){target="\_blank"} objects, so you need to access the individual elements.
...
What does this code do? It uses the [map_df](https://rdrr.io/cran/purrr/man/map_dfr.html){target="\_blank"} function in the _purrr_ package from tidyverse to cycle through each of the list elements in our `sim1` object. The _purrr_ package is all about working with lists in R. The `\(x)` is _purrr_'s shorthand way of defining a function of `x`, where `x` is the current member of the first argument to `map_df`, in this case, `sim1$data`, i.e., the output from our first simulated regimen. So we are looking at the first row of each `parValues` data field for each simulated output.
...
Notice the use of the pipe `%>%` or `|>` for plotly. This is an amazing innovation in Tidyverse (the magrittr package), which developed the `%>%` operator as a method of chaining the output of one function to the input of the next. [Tidyverse](https://www.tidyverse.org){target="\_blank"} is a powerful set of libraries for R that permit methodical, advanced data manipulation. You can load it with `library(tidyverse)`. Base R has now adopted the pipe, with one fewer characters to type. Any function in R which takes the data as the first argument, usually named `x`, will work with a pipe. See `?"%>%"` and `?"|>"` for more details on these useful operators.

**Plot the other simulated parameters on your own.**

## Summaries of simulations

[PM_sim](https://lapkb.github.io/Pmetrics/reference/PM_sim.html){target="\_blank"} objects contain a method to summarize the results, and there is a corresponding method for [PM_simlist](https://lapkb.github.io/Pmetrics/reference/PM_simlist.html){target="\_blank"} objects using the `at = ` argument that you have already seen with the `PM_sim$plot()` method.
...
To obtain the areas under the curve (AUC), you can use the `$auc` method attached to [PM_sim](https://lapkb.github.io/Pmetrics/reference/PM_sim.html){target="\_blank"} objects.
...
- Use the `$pta()` method attached to [PM_sim](https://lapkb.github.io/Pmetrics/reference/PM_sim.html){target="\_blank"} and [PM_simlist](https://lapkb.github.io/Pmetrics/reference/PM_simlist.html){target="\_blank"} objects.
- Use `PM_pta$new()`.
...
**Copy the code below into your R script** to create a PTA. Refer to [makePTA](https://lapkb.github.io/Pmetrics/reference/makePTA.html){target="\_blank"} for details on the arguments to the method.
...
pta1 <- sim1$pta(simlabels = simlabels,
                 target = c(0.25, 0.5, 1, 2, 4, 8, 16, 32, 64),
                 target_type = "time", free_fraction = 0.81,
...
pta2 <- sim1$pta(simlabels = simlabels,
                 target = makePTAtarget(pip_pseudo),
                 target_type = "time", free_fraction = 0.81,
                 success = 0.4, start = 48, end = 72)
...
* A vector of pharmacodynamic targets, such as Minimum Inhibitory Concentrations (MICs), e.g. c(0.25, 0.5, 1, 2, 4, 8, 16, 32). In the second PTA, instead of a uniform distribution for MIC, we used a sampled distribution using [makePTAtarget](https://lapkb.github.io/Pmetrics/reference/makePTAtarget.html){target="\_blank"} from real data (EUCAST).
* A single numerical value such as a concentration, e.g. 10.
* A sampled distribution using [makePTAtarget](https://lapkb.github.io/Pmetrics/reference/makePTAtarget.html){target="\_blank"}.
* A list of multiple targets combining the above if multiple target_types are used. If so, the first target can be a vector, but subsequent targets must be single values to avoid factorial expansion of combinations. For example, the first target could be a vector of MICs corresponding to a target_type of "time", the second target a value of 10 corresponding to a target_type of "min", and the third target a value of 50 corresponding to a target_type of "max": target = list(c(0.25, 0.5, 1, 2, 4, 8, 16, 32), 10, 50). The first value can also be a sampled distribution made with [makePTAtarget](https://lapkb.github.io/Pmetrics/reference/makePTAtarget.html){target="\_blank"}

- **target_type** A vector of the type for each target. For any, place a minus sign in front to make the success less than the target ratio, e.g. target_type = c("min", "-min"). Available types:
...
* "peak" or "max", ratio of peak/max (synonymous) concentration to target within the time range. Place a minus sign in front to make the success less than the target ratio.

* "min", is the ratio of minimum concentration to target within the time range. Place a minus sign in front to make the success less than the target ratio.
...
- **success** A vector specifying the success statistics, e.g. 0.4 for proportion time (end-start) above target, and/or 100 for max:target. For example `success = 0.4` or `success = c(0.4, 100)`. The length must be the same as for `target` and `target_type`.

The created objects `pta1` and `pta2` are [PM_pta](https://lapkb.github.io/Pmetrics/reference/PM_pta.html){target="\_blank"} objects, which you can look at by typing `pta1` or `pta2` into the R console. The output of makePTA is a list of class PMpta, which is a list with each `target_type` as an element, followed by a final intersection element showing the results for profiles which meet **ALL** the conditions (intersection) or NA if only one `target_type` was specified. The individual elements are tibbles with all possible combinations of targets and simulated regimens for a given `target_type.` The tibbles have the following columns:
...
`$intersect`⁠ item in the return list, the columns are the same, but the `target` and `target_type` will reflect all requested values expressed in parenthetical multiplication format to emphasize intersection, e.g., (auc)(min). Simulated (rather than discrete) targets made with [makePTAtarget](https://lapkb.github.io/Pmetrics/reference/makePTAtarget.html){target="\_blank"} will be abbreviated as "(sim)", e.g. (sim)(5) for a combination of simulated targets and a single concentration target of 5.

They also have several methods:

- **clone()** to make a copy.
- **save()** and **load()** to save/load as an .rds file.
- **plot()** to call [plot.PM_pta](https://lapkb.github.io/Pmetrics/reference/plot.PM_pta.html){target="\_blank"}.
- **summary()** to call [summary.PMpta](https://lapkb.github.io/Pmetrics/reference/summary.PMpta.html){target="\_blank"}
...
There are many ways to modify the plot, consistent with other Pmetrics plots which use the _plotly_ package and all detailed in the help for `plot.PM_data()`. For example,
...
The output is a tibble with the following columns (only the first three if `at = "intersect"`):
...
question("What will you have to change to accomplish this?",
         ...
         answer("predInt"),
         correct = "That's right. You can name your new poppar object as `poppar2` and your new model as `mod2`.",
         incorrect = "Try again. Which object contains the variances for the random parameters? Don't forget to adjust the model to increase the same variances for the random parameters.",
         allow_retry = TRUE)
```
...
Write the R code to make your new `poppar2` and `sim2`, copy it into your R script and run it. You can check your work with the _Solution_ button, but there are several ways to write this R code, so if yours doesn't match the solution we provided exactly, your code might still work!
...
For the model again use the `$clone()` and `$update` methods as below.
...
You should now have `dat2` and `mod3` which have both WT and CCR as random variables.

What needs to be done to `poppar`? Think about that for a minute, maybe try writing some code then check your answer. **Hint**, you can look at `mod3` and get the SD for CCR based on the range you created above, and of course $variance = SD^2$. Don't forget to inflate that SD by 2 before squaring it.
...
Try creating the simulation using these elements. Call it `sim3`. Then repeat the PTA (calling it `pta4`) as you did for `pta1` but using `sim3`.
...
- Keep `success = 1`
- Use `target = c(100, 120, 160)` to try differing toxcity thresholds
...
ptaTox <- sim1$pta(simlabels = simlabels,
                   target = c(100, 120, 160),
                   target_type = 72, free_fraction = 0.81,
                   success = 1, start = 48, end = 72)
